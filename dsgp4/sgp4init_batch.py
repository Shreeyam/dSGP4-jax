...
def initl_batch(
    ...
    x2o3   = jnp.full((batch_size,),2.0 / 3.0);
...
def sgp4init_batch(
    ...
    epoch (``jnp.float``): TLE days since 1949 December 31 00:00 UT
    xbstar (``jnp.float``): drag coefficient (in 1/earth_radii)
    xndot (``jnp.float``): first derivative of mean motion (in revs/day**2)
    xnddot (``jnp.float``): second derivative mean motion (in revs/day**3)
    xecco (``jnp.float``): eccentricity
    xargpo (``jnp.float``): argument of perigee [rad]
    xinclo (``jnp.float``): inclination [rad]
    xmo (``jnp.float``): mean anomaly [rad]
    xno_kozai (``jnp.float``): mean motion (radians/minute)
    xnodeo (``jnp.float``): right ascension of the ascending node [rad]
    ...
    satellite_batch._isimp   = jnp.full((batch_size,),0);   satellite_batch._method = ["n"]*batch_size;              satellite_batch._aycof    = jnp.full((batch_size,),0.0);
    satellite_batch._con41   = jnp.full((batch_size,),0.0); satellite_batch._cc1    = jnp.full((batch_size,),0.0); satellite_batch._cc4      = jnp.full((batch_size,),0.0);
    satellite_batch._cc5     = jnp.full((batch_size,),0.0); satellite_batch._d2     = jnp.full((batch_size,),0.0); satellite_batch._d3       = jnp.full((batch_size,),0.0);
    satellite_batch._d4      = jnp.full((batch_size,),0.0); satellite_batch._delmo  = jnp.full((batch_size,),0.0); satellite_batch._eta      = jnp.full((batch_size,),0.0);
    satellite_batch._argpdot = jnp.full((batch_size,),0.0); satellite_batch._omgcof = jnp.full((batch_size,),0.0); satellite_batch._sinmao   = jnp.full((batch_size,),0.0);
    satellite_batch._t       = jnp.full((batch_size,),0.0); satellite_batch._t2cof  = jnp.full((batch_size,),0.0); satellite_batch._t3cof    = jnp.full((batch_size,),0.0);
    satellite_batch._t4cof   = jnp.full((batch_size,),0.0); satellite_batch._t5cof  = jnp.full((batch_size,),0.0); satellite_batch._x1mth2   = jnp.full((batch_size,),0.0);
    satellite_batch._x7thm1  = jnp.full((batch_size,),0.0); satellite_batch._mdot   = jnp.full((batch_size,),0.0); satellite_batch._nodedot  = jnp.full((batch_size,),0.0);
    satellite_batch._xlcof   = jnp.full((batch_size,),0.0); satellite_batch._xmcof  = jnp.full((batch_size,),0.0); satellite_batch._nodecf   = jnp.full((batch_size,),0.0);
  ...
  # -------------------------------------------------------------------------
    ...
    satellite_batch._error = jnp.full((batch_size,),0)
  ...
  # sgp4fix rename variables to clarify which mean motion is intended
    ...
    satellite_batch._am = jnp.full((batch_size,),0.0)
    satellite_batch._em = jnp.full((batch_size,),0.0)
    satellite_batch._im = jnp.full((batch_size,),0.0)
    satellite_batch._Om = jnp.full((batch_size,),0.0)
    satellite_batch._mm = jnp.full((batch_size,),0.0)
    satellite_batch._nm = jnp.full((batch_size,),0.0)
    ...
    satellite_batch._a    = jnp.power( satellite_batch._no_unkozai*satellite_batch._tumin , (-2.0/3.0) )
    ...
    satellite_batch._isimp = jnp.where(condition1, jnp.array(0), satellite_batch._isimp)
    condition2 = condition1 & (rp < 220.0 / satellite_batch._radiusearthkm + 1.0)
    satellite_batch._isimp = jnp.where(condition2, jnp.array(1), satellite_batch._isimp)

    sfour = jnp.where(condition1, ss, jnp.zeros_like(ss))
    qzms24 = jnp.where(condition1, qzms2t, jnp.zeros_like(qzms2t))
    perige = jnp.where(condition1, (rp - 1.0) * satellite_batch._radiusearthkm, jnp.zeros_like(rp))

    condition3 = condition1 & (perige < 156.0)
    sfour_temp1 = jnp.where(perige < 98.0, jnp.array(20.0), perige - 78.0)
    sfour = jnp.where(condition3, sfour_temp1, sfour)
    qzms24temp = (120.0 - sfour) / satellite_batch._radiusearthkm
    qzms24 = jnp.where(condition3, qzms24temp ** 4, qzms24)
    sfour = jnp.where(condition3, sfour / satellite_batch._radiusearthkm + 1.0, sfour)
    ...
    cc3 = jnp.where(satellite_batch._ecco > 1.0e-4,
    ...
    satellite_batch._cc4 = 2.0 * satellite_batch._no_unkozai * coef1 * ao * omeosq * \
                        (satellite_batch._eta * (2.0 + 0.5 * etasq) + satellite_batch._ecco *
                            ...
                            (2.0 * etasq - eeta * (1.0 + etasq)) * jnp.cos(2.0 * satellite_batch._argpo)))
    ...
    satellite_batch._omgcof = satellite_batch._bstar * cc3 * jnp.cos(satellite_batch._argpo)
    satellite_batch._xmcof = jnp.where(satellite_batch._ecco > 1.0e-4,
    ...
    satellite_batch._xlcof = jnp.where(jnp.abs(cosio + 1.0) > 1.5e-12,
    ...
    delmotemp = 1.0 + satellite_batch._eta * jnp.cos(satellite_batch._mo)
    satellite_batch._delmo = delmotemp ** 3
    satellite_batch._sinmao = jnp.sin(satellite_batch._mo)
    satellite_batch._x7thm1 = 7.0 * cosio2 - 1.0

    # Deep space initialization
    deep_space_condition = (2 * np.pi / satellite_batch._no_unkozai >= 225.0)
    if jnp.any(deep_space_condition):
    ...
    if jnp.any(isimp_condition):
        ...
        satellite_batch._d2 = jnp.where(isimp_condition, d2, satellite_batch._d2)
        satellite_batch._d3 = jnp.where(isimp_condition, d3, satellite_batch._d3)
        satellite_batch._d4 = jnp.where(isimp_condition, d4, satellite_batch._d4)
        satellite_batch._t3cof = jnp.where(isimp_condition, t3cof, satellite_batch._t3cof)
        satellite_batch._t4cof = jnp.where(isimp_condition, t4cof, satellite_batch._t4cof)
        satellite_batch._t5cof = jnp.where(isimp_condition, t5cof, satellite_batch._t5cof)
    sgp4_batched(satellite_batch, jnp.zeros((batch_size,)))
