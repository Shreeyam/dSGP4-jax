...
class UtilTestCase(unittest.TestCase):
    def test_newton_method_1(self):
        ...
        my_tle=dsgp4_jax.tle.TLE(lines)
        gravity_constant_name="wgs-84"
        whichconst=dsgp4_jax.util.get_gravity_constants(gravity_constant_name)
        dsgp4_jax.sgp4init(whichconst=whichconst,
        ...
        found_tle,_=dsgp4_jax.newton_method(tle0=my_tle, time_mjd=dsgp4_jax.util.from_datetime_to_mjd(my_tle._epoch),gravity_constant_name=gravity_constant_name)
        ...
        tsince_2=1000.
        new_tol=1e-13
        target_state=dsgp4_jax.sgp4(my_tle,tsince_2*np.ones((1,1)))
        time_mjd_0=dsgp4_jax.util.from_datetime_to_mjd(my_tle._epoch)
        time_mjd=time_mjd_0+tsince_2/24/60
        found_tle_2,_=dsgp4_jax.newton_method(tle0=my_tle, time_mjd=time_mjd, new_tol=new_tol, target_state=target_state,gravity_constant_name=gravity_constant_name)
        found_state=dsgp4_jax.sgp4(found_tle_2,np.array((time_mjd-dsgp4_jax.util.from_datetime_to_mjd(found_tle_2._epoch))*1440.))
